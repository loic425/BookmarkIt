...
class InstallCommand extends AbstractInstallCommand
{
    ...
    protected function configure()
    {
        $this
            ->setName('app:install')
            ->setDescription('Installs BookmarkIt in your preferred environment.')
            ->setHelp(<<<EOT
The <info>%command.name%</info> command installs BookmarkIt.
EOT
            );
    }
    ...
    {
        $output->writeln('<info>Installing BookmarkIt...</info>');
    ...
    {
        if ($errored) {
            return '<info>BookmarkIt has been installed, but some error occurred.</info>';
        }

        return '<info>BookmarkIt has been successfully installed.</info>';
    }
