...
get_bookmark_it_path()
{
    echo "$(cd "$(dirname "${BASH_SOURCE[0]}")/../../" && pwd)"
}

has_bookmark_it_cache() {
    if [[ ! -z "${BOOKMARK_IT_CACHE_DIR}" && -d "${BOOKMARK_IT_CACHE_DIR}" ]]; then
        return 0
    else
        return 1
    fi
...
inform_about_bookmark_it_cache() {
    if ! has_bookmark_it_cache; then
        print_warning "Sylius cache should be used, but it is not configured correctly."
        print_warning "Check whether you have \$BOOKMARK_IT_CACHE_DIR set and if that directory exists."
    fi
}
...
is_package_cache_fresh() {
    ...
    if [[ -f "${BOOKMARK_IT_CACHE_DIR}/composer-${cache_key}.lock" && -f "${BOOKMARK_IT_CACHE_DIR}/composer-${cache_key}.json.md5sum" ]]; then
        current_hash="$(file_md5sum "${package_path}/composer.json")"
        cached_hash="$(cat "${BOOKMARK_IT_CACHE_DIR}/composer-${cache_key}.json.md5sum")"
