BookmarkIt Documentation
=====================

.. image:: /_images/logo_big.png
   :alt:   BookmarkIt Welcome Page

`BookmarkIt`_ is a project based on
`Symfony Framework`_ and `Sylius`_.

.. note::

    This documentation assumes you have a working knowledge of the Symfony
    Framework. If you're not familiar with Symfony, please start with
    reading the `Quick Tour`_ from the Symfony documentation.

The Book
--------

Here you will find all the concepts used in the BookmarkIt platform.
:doc:`The Book </book/index>` helps to understand how BookmarkIt works.

.. toctree::
    :hidden:

    book/index

.. include:: /book/map.rst.inc

The REST API Reference
----------------------

:doc:`The API guide </api/index>` covers the REST API of Sylius platform.

.. toctree::
    :hidden:

    api/index

.. include:: /api/map.rst.inc


Deployment
----------

:doc:`The Deployment guide </deployment/index>` helps to deploy the website on servers.

.. toctree::
    :hidden:

    deployment/index

.. include:: /deployment/map.rst.inc

The Cookbook
------------

:doc:`The Cookbook </cookbook/index>` is a collection of specific solutions for specific needs.

.. toctree::
    :hidden:

    cookbook/index

.. include:: /cookbook/map.rst.inc

The Behat Guide
---------------

In :doc:`the Behat Guide </behat/index>` you will learn how to write clean and reusable features, contexts and pages.

.. toctree::
    :hidden:

    behat/index

.. include:: /behat/map.rst.inc


.. _BookmarkIt: http://www.mobizel.com
.. _Sylius: http://sylius.org
.. _`Symfony Framework`: http://symfony.com
.. _`Quick Tour`: http://symfony.com/doc/current/quick_tour

